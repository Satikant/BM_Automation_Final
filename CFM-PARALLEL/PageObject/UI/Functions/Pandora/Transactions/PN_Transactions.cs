...
CommonUtilities.Logout(Driver);       Driver.Quit();
        ...
        public void VerifyingAccrualEntryUnderDetailedReport(string accrualType="Flat")
        {
            ...
            try
            {
                ...
                Assert.AreEqual(regex.Replace(bi.GetText(obj_transaction.FirstrowAmount), ""), regex.Replace(Parameters.Pandora_AccrualPositive_Amount,""));
        ...
        public void VerifyingTransferEntryUnderDetailedReport(string accrualType = "Flat")
        {
            ...
            try
            {
                ...
                bi.WaitTime(10);
                Regex regex = new Regex("[^0-9.]");
                Assert.True(bi.GetText(obj_transaction.FirstrowAmount).Contains("("));
                Console.WriteLine("Application showing The Specified amount is deducted from the Source LME");
                Assert.AreEqual(Convert.ToDouble(regex.Replace(bi.GetText(obj_transaction.FirstrowAmount), "")), Convert.ToDouble(regex.Replace(Parameters.Pandora_TransferPositive_Amount, "")));
                //Checking the First row Amount
                Console.WriteLine("Accrual Amount Entry showing under Fund Snapshot");
            }
            catch (Exception )
        ...
        public void VerifyingAdjustmentEntryUnderDetailedReport(string accrualType = "Flat")
        {
            ...
            try
            {
                ...
                //Console.WriteLine("Application showing The Specified amount is deducted from the Source LME");
                Assert.AreEqual(Convert.ToDouble(regex.Replace(bi.GetText(obj_transaction.FirstrowAmount), "")), Convert.ToDouble(regex.Replace(Parameters.Pandora_AccrualPositive_Amount, "")));
                //Checking the First row Amount
                Console.WriteLine("Accrual Amount Entry showing under Fund Snapshot");
            }
            catch (Exception ex)
